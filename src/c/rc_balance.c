...
void* __setpoint_manager(__attribute__ ((unused)) void* ptr)
{
	...
	while(rc_get_state()!=EXITING){
		...
		char right_str[20] = "right.txt.";
		char break_str[20] = "break.txt.";
		char up_file[70] = "";
		char down_file[70] = "";
		char left_file[70] = "";
		char right_file[70] = "";
		char break_file[70] = "";
		strncat(up_file, var_file_dir, 70);
		strncat(down_file, var_file_dir, 70);
		strncat(left_file, var_file_dir, 70);
		strncat(right_file, var_file_dir, 70);
		strncat(break_file, var_file_dir, 70);
		strncat(up_file, up_str, 70);
		strncat(down_file, down_str, 70);
		strncat(left_file, left_str, 70);
		strncat(right_file, right_str, 70);
		strncat(break_file, break_str, 70);
		// if dsm is active, update the setpoint rates
		switch(m_input_mode){
		case NONE:
			continue;
		case DSTR:
			if ((file = fopen(up_file, "r")))
		    {
		        printf(up_str);
		        remove(file);
			    fclose(file);
